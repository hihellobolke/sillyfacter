...
#
#
# This was further modified to enable to work with sillyfacter
# - 2014, HiHelloBolke@gmail.com
#
...
try:
    import sillyfacter.custom.ansiblelibs.selinux
    HAVE_SELINUX = True
except ImportError:
    HAVE_SELINUX = False
...
class Facts(object):
    ...
    def get_platform_facts(self):
        ...
        if self.facts['system'] == 'Linux':
            ...
            rc, out, err = run_command("/usr/sbin/bootinfo -p")
            data = out.split('\n')
            self.facts['architecture'] = data[0]

    _ = '''def get_local_facts(self):
        fact_path = params.get('fact_path', None)
    ...
    def get_distribution_facts(self):
        ...
        if self.facts['system'] == 'AIX':
            ...
            rc, out, err = run_command("/usr/bin/oslevel")
            data = out.split('.')
            ...
            rc, out, err = run_command(
                "/usr/sbin/swlist |egrep 'HPUX.*OE.*[AB].[0-9]+\.[0-9]+'")
            data = re.search(
            ...
            rc, out, err = run_command("/usr/bin/sw_vers -productVersion")
            data = out.split()[-1]
            ...
            self.facts['distribution_release'] = platform.release()
            rc, out, err = run_command("/sbin/sysctl -n kern.version")
    ...
    def get_lsb_facts(self):
        ...
        if lsb_path:
            rc, out, err = run_command([lsb_path, "-a"])
            if rc == 0:
                self.facts['lsb'] = {}
    ...
    def get_selinux_facts(self):
        ...
        if not selinux.is_selinux_enabled():
            ...
            try:
                ...
                if rc == 0:
                    self.facts['selinux'][
                        'config_mode'] = Facts.SELINUX_MODE_DICT.get(configmode, 'unknown')
                else:
    ...
    def get_date_time_facts(self):
        ...
        self.facts['date_time'][
            'iso8601_micro'] = now.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ")
        self.facts['date_time'][
            'iso8601'] = now.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
...
class LinuxHardware(Hardware):
        ...
        if os.path.exists('/sys/devices/virtual/dmi/id/product_name'):
            ...
            for (k, v) in DMI_DICT.items():
                if dmi_bin is not None:
                    (rc, out, err) = run_command('%s -s %s' % (dmi_bin, v))
                    if rc == 0:
    ...
    def get_device_facts(self):
        self.facts['devices'] = {}
        lspci = get_bin_path('lspci')
        if lspci:
            rc, pcidata, err = run_command([lspci, '-D'])
        else:
            pcidata = None
...
class SunOSHardware(Hardware):
    ...
    def get_cpu_facts(self):
        physid = 0
        sockets = {}
        rc, out, err = run_command("/usr/bin/kstat cpu_info")
        self.facts['processor'] = []
        for line in out.split('\n'):
            if len(line) < 1:
                continue
    ...
    def get_memory_facts(self):
        rc, out, err = run_command(["/usr/sbin/prtconf"])
        for line in out.split('\n'):
            if 'Memory size' in line:
                self.facts['memtotal_mb'] = line.split()[2]
        rc, out, err = run_command("/usr/sbin/swap -s")
...
class OpenBSDHardware(Hardware):
    ...
    def get_sysctl(self):
        rc, out, err = run_command(["/sbin/sysctl", "hw"])
        if rc != 0:
            return dict()
        sysctl = dict()
    ...
    def get_memory_facts(self):
        ...
        # 0  1 99
        rc, out, err = run_command("/usr/bin/vmstat")
        if rc == 0:
        ...
        rc, out, err = run_command("/sbin/swapctl -sk")
        if rc == 0:
            data = out.split()
    ...
    def get_processor_facts(self):
        ...
        if not dmesg_boot:
            rc, dmesg_boot, err = run_command("/sbin/dmesg")
        i = 0
...
class FreeBSDHardware(Hardware):
    ...
    def get_cpu_facts(self):
        self.facts['processor'] = []
        rc, out, err = run_command("/sbin/sysctl -n hw.ncpu")
    ...
    def get_memory_facts(self):
        rc, out, err = run_command("/sbin/sysctl vm.stats")
        for line in out.split('\n'):
            data = line.split()
        ...
        #
        rc, out, err = run_command("/usr/sbin/swapinfo -m")
        lines = out.split('\n')
        if len(lines[-1]) == 0:
            lines.pop()
        data = lines[-1].split()
    ...
    def get_device_facts(self):
        ...
        drives = re.compile('(ada?\d+|da\d+|a?cd\d+)')
                            # TODO: rc, disks, err = run_command("/sbin/sysctl
                            # kern.disks")
...
class AIX(Hardware):
    ...
    def get_cpu_facts(self):
        ...
        rc, out, err = run_command("/usr/sbin/lsdev -Cc processor")
        if out:
            i = 0
            for line in out.split('\n'):
...
class HPUX(Hardware):
    ...
    def get_cpu_facts(self):
        if self.facts['architecture'] == '9000/800':
            rc, out, err = run_command("ioscan -FkCprocessor|wc -l")
        ...
        # Working with machinfo mess
            ...
            if self.facts['distribution_version'] == "B.11.31":
                ...
                if out.strip() == '0':
                    rc, out, err = run_command(
                        "/usr/contrib/bin/machinfo |grep Intel")
                    self.facts['processor_count'] = int(
                        out.strip().split(" ")[0])
                    # If hyperthreading is active divide cores by 2
                    rc, out, err = run_command("/usr/sbin/psrset |grep LCPU")
    ...
    def get_memory_facts(self):
        pagesize = 4096
        rc, out, err = run_command("/usr/bin/vmstat|tail -1")
        ...
        rc, out, err = run_command("/usr/sbin/swapinfo -m -d -f -q")
        self.facts['swaptotal_mb'] = int(out.strip())
        rc, out, err = run_command(
            "/usr/sbin/swapinfo -m -d -f |egrep '^dev|^fs'")
        swap = 0
    ...
    def get_default_interfaces(self, ip_path):
        ...
        for v in 'v4', 'v6':
            ...
            if v == 'v6' and not socket.has_ipv6:
                continue
            rc, out, err = run_command(command[v])
            if not out:
...
class GenericBsdIfconfigNetwork(Network):
    ...
    def populate(self):
        ...
        ifconfig_path = get_bin_path('ifconfig')

        if ifconfig_path is None:
            return self.facts
        route_path = get_bin_path('route')
...
class HPUXVirtual(Virtual):
    ...
    def get_virtual_facts(self):
        if os.path.exists('/usr/sbin/vecheck'):
            rc, out, err = run_command("/usr/sbin/vecheck")
            if rc == 0:
                self.facts['virtualization_type'] = 'guest'
                self.facts['virtualization_role'] = 'HP vPar'
        if os.path.exists('/opt/hpvm/bin/hpvminfo'):
            rc, out, err = run_command("/opt/hpvm/bin/hpvminfo")
        ...
        if os.path.exists('/usr/sbin/parstatus'):
            rc, out, err = run_command("/usr/sbin/parstatus")
            if rc == 0:
                self.facts['virtualization_type'] = 'guest'
...
class SunOSVirtual(Virtual):
    ...
    def get_virtual_facts(self):
        rc, out, err = run_command("/usr/sbin/prtdiag")
        for line in out.split('\n'):
            if 'VMware' in line:
                self.facts['virtualization_type'] = 'vmware'
                self.facts['virtualization_role'] = 'guest'
        ...
        if os.path.exists("/usr/bin/zonename"):
            rc, out, err = run_command("/usr/bin/zonename")
        ...
        if 'container' in self.facts and self.facts['container'] == 'zone':
            rc, out, err = run_command("/usr/sbin/modinfo")
            for line in out.split('\n'):
                if 'VMware' in line:
                    self.facts['virtualization_type'] = 'vmware'
                    self.facts['virtualization_role'] = 'guest'
...
def fetch(host=platform.node()):
    ...
    #
    l.info("Gathering Ansible_facts...")
    retval = {}
    retval["custom"] = {"ansible": {}}
    ret = {}
    #ret = retval["custom"]["ansible"]
    try:
        ret = ansible_facts()
    except:
        ...
        if sillyfacter.config.DEBUG:
            l.exception("Failed to gather anible facts")
        l.error("error when executing ansible_facts")
        ret["ansible_present"] = False
    else:
        ret["ansible_present"] = True
    retval["custom"]["ansible"] = ret
    return retval
...
def output(f):
    ...
    l = logging.LoggerAdapter(fetch_lg(), ldict)
    #
    l.info("Generating Ansible custom output")
    retval = {"ansible": {}}
    fetched = retval["ansible"]
    try:
        for item, val in f["custom"]["ansible"].iteritems():
            fetched[item] = val
    except:
        if sillyfacter.config.STRICT:
            raise
        if sillyfacter.config.DEBUG:
            l.exception("Failed to generate output")
        pass
    return retval
